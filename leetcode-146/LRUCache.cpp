#include <iostream>
#include <map>
#include <vector>
#include <string>
using namespace std;

vector<int> Batch(vector<string> op, vector<vector<int>> data);
struct Val{
	int value_;
	int NextKey;
	int PrevKey;
};

class LRUCache {
public:
	LRUCache(int capacity)
	{
		maxBufferSize = capacity;
		elems = 0;
		headKey = -1;
		tailKey = -1;
	}

	int get(int key) {
		auto iter = buffers.find(key);
		if (iter == buffers.end()) {
			return -1;
		} 

		RelocatKey(key);
		return iter->second.value_;
	}

	void RelocatKey(int key) {
		if (key == tailKey) {
			return;
		}
		auto iter = buffers.find(key);
		int nextKey = iter->second.NextKey;
		int prevKey = iter->second.PrevKey;

		buffers[nextKey].PrevKey = prevKey;
		buffers[tailKey].NextKey = key;
		buffers[key].PrevKey = tailKey;
		tailKey = key;

		if (key == headKey) {
			headKey = nextKey;
		}
		else {
			buffers[prevKey].NextKey = nextKey;
		}
	}
	void put(int key, int value) {
		Val v = { value, -1,-1 };
		if (headKey == -1) {
			buffers[key] = v;
			headKey = key;
			tailKey = key;
			elems = 1;
		}
		else {
			auto iter = buffers.find(key);
			if (iter == buffers.end()) {
				v.PrevKey = tailKey;
				buffers[tailKey].NextKey = key;
				tailKey = key;
				buffers[key] = v;
				if (elems == maxBufferSize) {
					int k = buffers[headKey].NextKey;
					buffers.erase(headKey);
					headKey = k;
					buffers[k].PrevKey = -1;
				}
				else {
					elems++;
				}
			}
			else {
				buffers[key].value_ = value;
				RelocatKey(key);
			}

			
		}
	}

	int GetSize() {
		return buffers.size();
	}

	bool CheckFluent() { 
		std::cout << "key forward: " << endl;
		int key = headKey;
		auto IsEleExist = [&](int k) {
			auto i = buffers.find(k);
			return i != buffers.end();
		};
		for (int i = 0; i < elems; i++) {
			if (key != -1 && !IsEleExist(key)) {
				return false;
			}
			std::cout << key << " " ;
			key = buffers[key].NextKey;
		}

		cout << endl;
		std::cout << "key back: " << endl;
		key = tailKey;
		for (int i = 0; i < elems; i++) {
			if (key != -1 && !IsEleExist(key)) {
				return false;
			}
			std::cout << key << " ";
			key = buffers[key].PrevKey;
		}
		cout << endl;
		cout << "buffer size: " << buffers.size() << endl;
		return true;
	}
private:
	
	map<int, Val> buffers;
	int headKey;
	int tailKey;
	int maxBufferSize;
	int elems;
};
int main()
{
	/*
	["LRUCache", "put", "put", "put", "put", "put", "get", "put", "get", "get", "put", "get", "put", "put", "put", "get", "put", "get", "get", "get", "get", "put", "put", "get", "get", "get", "put", "put", "get", "put", "get", "put", "get", "get", "get", "put", "put", "put", "get", "put", "get", "get", "put", "put", "get", "put", "put", "put", "put", "get", "put", "put", "get", "put", "put", "get", "put", "put", "put", "put", "put", "get", "put", "put", "get", "put", "get", "get", "get", "put", "get", "get", "put", "put", "put", "put", "get", "put", "put", "put", "put", "get", "get", "get", "put", "put", "put", "get", "put", "put", "put", "get", "put", "put", "put", "get", "get", "get", "put", "put", "put", "put", "get", "put", "put", "put", "put", "put", "put", "put"]
	[[10], [10, 13], [3, 17], [6, 11], [10, 5], [9, 10], [13], [2, 19], [2], [3], [5, 25], [8], [9, 22], [5, 5], [1, 30], [11], [9, 12], [7], [5], [8], [9], [4, 30], [9, 3], [9], [10], [10], [6, 14], [3, 1], [3], [10, 11], [8], [2, 14], [1], [5], [4], [11, 4], [12, 24], [5, 18], [13], [7, 23], [8], [12], [3, 27], [2, 12], [5], [2, 9], [13, 4], [8, 18], [1, 7], [6], [9, 29], [8, 21], [5], [6, 30], [1, 12], [10], [4, 15], [7, 22], [11, 26], [8, 17], [9, 29], [5], [3, 4], [11, 30], [12], [4, 29], [3], [9], [6], [3, 4], [1], [10], [3, 29], [10, 28], [1, 20], [11, 13], [3], [3, 12], [3, 8], [10, 9], [3, 26], [8], [7], [5], [13, 17], [2, 27], [11, 15], [12], [9, 19], [2, 15], [3, 16], [1], [12, 17], [9, 1], [6, 19], [4], [5], [5], [8, 1], [11, 7], [5, 2], [9, 28], [1], [2, 2], [7, 4], [4, 22], [7, 24], [9, 26], [13, 28], [11, 26]]
	*/
	vector<string> op = { "LRUCache", "put", "put", "put", "put", "put", "get", "put", "get", "get", "put", "get", "put", "put", "put", "get", "put", "get", "get", "get", "get", "put", "put", "get", "get", "get", "put", "put", "get", "put", "get", "put", "get", "get", "get", "put", "put", "put", "get", "put", "get", "get", "put", "put", "get", "put", "put", "put", "put", "get", "put", "put", "get", "put", "put", "get", "put", "put", "put", "put", "put", "get", "put", "put", "get", "put", "get", "get", "get", "put", "get", "get", "put", "put", "put", "put", "get", "put", "put", "put", "put", "get", "get", "get", "put", "put", "put", "get", "put", "put", "put", "get", "put", "put", "put", "get", "get", "get", "put", "put", "put", "put", "get", "put", "put", "put", "put", "put", "put", "put" };
	vector<vector<int>> data = { {10},{10, 13},{3, 17},{6, 11},{10, 5},{9, 10},{13},{2, 19},{2},{3},{5, 25},{8},{9, 22},{5, 5},{1, 30},{11},{9, 12},{7},{5},{8},{9},{4, 30},{9, 3},{9},{10},{10},{6, 14},{3, 1},{3},{10, 11},{8},{2, 14},{1},{5},{4},{11, 4},{12, 24},{5, 18},{13},{7, 23},{8},{12},{3, 27},{2, 12},{5},{2, 9},{13, 4},{8, 18},{1, 7},{6},{9, 29},{8, 21},{5},{6, 30},{1, 12},{10},{4, 15},{7, 22},{11, 26},{8, 17},{9, 29},{5},{3, 4},{11, 30},{12},{4, 29},{3},{9},{6},{3, 4},{1},{10},{3, 29},{10, 28},{1, 20},{11, 13},{3},{3, 12},{3, 8},{10, 9},{3, 26},{8},{7},{5},{13, 17},{2, 27},{11, 15},{12},{9, 19},{2, 15},{3, 16},{1},{12, 17},{9, 1},{6, 19},{4},{5},{5},{8, 1},{11, 7},{5, 2},{9, 28},{1},{2, 2},{7, 4},{4, 22},{7, 24},{9, 26},{13, 28},{11, 26}};
	auto r = Batch(op, data);
	for (auto v : r) {
		std::cout << v << " ";
	}
	cout << endl;
}

vector<int> Batch(vector<string> op, vector<vector<int>> data)
{
	int k = 0;
	vector<int> v;
	if (op.size() != data.size() ) {
		return v;
	}
	
	LRUCache *obj = nullptr; 
	int v_ = 0;
	for (int i = 0; i < op.size(); i++)
	{
		if (op[i] == "LRUCache") {
			v_ = data[i][0];
			obj = new LRUCache(v_);
		}
		
		if (op[i] == "put") {
			if (data[i].size() != 2) {
				continue;
			}

			obj->put(data[i][0], data[i][1]);
		}

		if (op[i] == "get") {
			if (data[i].size() != 1) {
				continue;
			}
			k = obj->get(data[i][0]);
			if (k == 24) {
				std::cout << "kkkkkk" << std::endl;
			}
			v.push_back(k);
		}
		std::cout << "op:" << op[i] << " " << data[i][0];
		if (op[i] == "put") {
			cout << " " << data[i][1] << endl;
		}
		else {
			cout << endl;
		}

		obj->CheckFluent();
	}

	return v;
}
